...
namespace _Project.Scripts.DomainObjects.Rules
{
    public class SymmetryRule : Rule
    {
        ...
        public bool IsInvalidated(List<Bone> leftBoneObjects, List<Bone> rightBoneObjects, Bone referenceBone)
        {
            ...
            {
                var flippedVector = RotateAround(leftBoneObject.boneVector, referenceBone.boneVector,
                    referenceBone.boneVector, 180);
        ...
        {
            var vector3 = Quaternion.AngleAxis(angle, aroundAxis) * (fromPoint - aroundPoint);
